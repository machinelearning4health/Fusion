...
class Batch:
    ...
    def add_instance(self, row, ind2c, c2ind, w2ind, dv_dict, num_labels):
        """
            Makes an instance to add to this batch from given row mimicdata, with a bunch of lookups
        """
        labels = set()
        hadm_id = int(row[1])
        text = row[2]
...
def data_generator(filename, dicts, batch_size, num_labels, desc_embed=False, version='mimic3'):
    """
        Inputs:
            filename: holds mimicdata sorted by sequence length, for best batching
        ...
        Yields:
            np arrays with mimicdata for training loop.
    """
...
def reformat(code, is_diag):
    """
        Put a period in the right place because the MIMIC-3 mimicdata files exclude them.
...
def load_code_descriptions(version='mimic3'):
    #load description lookup from the appropriate mimicdata files
...
def get_pcodes(hids, c2ind):
    ...
    return pcodes, pcodes_mask_final
