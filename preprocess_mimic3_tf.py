...
from utils import build_vocab, word_embeddings, fasttext_embeddings, gensim_to_fasttext_embeddings, gensim_to_embeddings, \
    reformat, write_discharge_summaries, concat_data, split_data_tf
...
fname_raw = '%s/notes_labeled_original.csv' % args.MIMIC_3_DIR
base_name = "%s/disch" % args.MIMIC_3_DIR #for output
tr, dv, te = split_data_tf(fname, fname_raw, base_name, args.MIMIC_3_DIR)
...
for splt in ['train', 'dev', 'test']:
    filename = '%s/disch_%s_split_tf.csv' % (args.MIMIC_3_DIR, splt)
    df = pd.read_csv(filename)
    df['length'] = df.apply(lambda row: len(str(row['TEXT']).split()), axis=1)
    df = df.sort_values(['length'])
    df.to_csv('%s/%s_full_tf.csv' % (args.MIMIC_3_DIR, splt), index=False)
